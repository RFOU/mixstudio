...
import { Music2, Mail, Lock, Eye, EyeOff, X } from 'lucide-react'
...
export function AuthModal({ onClose, redirectTo }: AuthModalProps) {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-sm mx-4 rounded-xl p-6 shadow-2xl relative"
      ...
      >
        ...
        {/* Mode toggle */}
        <div className="flex rounded-lg p-1 mb-4" style={{ background: 'var(--surface-2)' }}>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2"
                style={{ color: 'var(--text-muted)' }} onClick={() => setShowPassword(v => !v)}>
          ...
          <Button type="submit" variant="default" size="lg" className="w-full" disabled={loading || !!oauthLoading}>
            {loading ? 'Chargement...' : mode === 'login' ? 'Se connecter' : 'Créer un compte'}
          </Button>
        </form>

        {onClose && (
          <button className="mt-4 w-full text-xs text-center" style={{ color: 'var(--text-muted)' }} onClick={onClose}>
